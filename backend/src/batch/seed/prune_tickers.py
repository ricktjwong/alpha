from repositories import TickerRepository, TickerOHLCRepository
...
def prune_tickers(db):
    ...
    db.session.query(Ticker).filter(Ticker.symbol.in_(diff_symbols)).delete(synchronize_session=False)
    db.session.commit()
